...
export function MarkdownRenderer({ children }: MarkdownRendererProps) {
  const processedText = rewriteSandboxImageMarkdown(children.replace(/\\n/g, '\n'));
...
function resolveSandboxImageSrc(src: unknown) {
  if (typeof src !== 'string') return src;

  const threadId = getCurrentThreadId();
  if (!threadId) return src;

  let sandboxPath: string | undefined;

  if (src.startsWith('sandbox:')) {
    sandboxPath = src.replace(/^sandbox:/, '');
  } else if (src.startsWith('file:///workspace/')) {
    sandboxPath = src.replace(/^file:\/\//, '');
    ...
    sandboxPath = `/workspace/${src.replace(/^\.\//, '')}`;
  }

  if (!sandboxPath) return src;

  return `/manifex/threads/${encodeURIComponent(threadId)}/artifacts?path=${encodeURIComponent(sandboxPath)}`;
}

function rewriteSandboxImageMarkdown(text: string) {
  return text.replace(/!\[([\s\S]*?)\]\(([^)\s]+)\)/g, (match, alt: string, src: string) => {
    const resolved = resolveSandboxImageSrc(src);
    if (typeof resolved !== 'string' || resolved === src) return match;
    return `![${alt}](<${resolved}>)`;
  });
}
...
const COMPONENTS: Components = {
  ...
  img: ({ src, alt, className, ...props }) => (
    <img
      {...props}
      src={resolveSandboxImageSrc(src) as string | undefined}
      alt={alt ?? ''}
      className={cn('max-h-[420px] max-w-full rounded-md border border-border1 object-contain', className)}
    />
  ),
