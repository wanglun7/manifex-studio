...
const scopedAtAliasPlugin = (): Plugin => {
...
export default defineConfig(({ mode }) => {
  const commonConfig: UserConfig = {
    plugins: [scopedAtAliasPlugin(), stubNodeBuiltinsPlugin, tailwindcss(), react(), routesManifestPlugin()],
    base: './',
    resolve: {
      ...
      alias: [
        {
          find: '@internal-temp',
          replacement: path.resolve(__dirname, './src/vendor/@mastra'),
        },
        {
          find: /^@internal\/core\/(.+)$/,
  ...
  if (mode === 'development') {
    // Proxy Studio-only routes to the Mastra runtime, not the Vite dev server.
    const targetPort = process.env.MASTRA_SERVER_PORT || process.env.MASTRA_PORT || '4111';
    const targetHost = process.env.MASTRA_SERVER_HOST || process.env.MASTRA_HOST || 'localhost';
