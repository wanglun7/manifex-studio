...
import { ImageEntry, PdfEntry, TxtEntry } from './attachment-preview-dialog';
...
const ComposerPdfAttachment = ({ attachment }: { attachment: ComposerAttachment }) => {
  ...
  return (
    <div className="flex h-full w-full items-center justify-center">
      {state.isLoading ? <Spinner /> : <PdfEntry data={state.text} url={attachment.isUrl ? attachment.name : undefined} />}
    </div>
  );
};
